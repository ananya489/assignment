...
| Task | Command / action |
|---|---|
| Go to project | `cd /workspaces/assignment` |
| Install library | `python -m pip install kafka-python` |
| Check library | `python -c "import kafka; print(kafka.__version__)"` |
| Start Kafka | `cd /workspaces/assignment/kafka_2.13-4.3.1 && bin/kafka-server-start.sh config/server.properties` |
| Create topic | `python topic.py` |
| Verify topic | `bin/kafka-topics.sh --list --bootstrap-server localhost:9092` (from Kafka folder) |
| Start consumer | `python consumer.py` |
| Send 10 messages | `python Producer.py` |
| Verify records | `bin/kafka-console-consumer.sh --topic server_metrics --from-beginning --bootstrap-server localhost:9092` (from Kafka folder) |
